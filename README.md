...
WebRTC это набор спецификаций, описывающих трансляцию аудио\видео и сообщений (data streams) в режиме реального времени. Каждый раз обращаться к WebRTC как к набору спецификаций не слишком удобно, поэтому дальше в тексте я буду явно писать WebRTC, спецфикация, или просто протокол, в зависимости от контекста.
...
что браузер будет обладать всей функциональностью, достаточной для того,
чтобы выступать в роли телефона и юнита конференц-связи;
цель в том чтобы предоставить web-приложению функционал,
достаточный, чтобы работая совместно с бэкендом, реализовать
